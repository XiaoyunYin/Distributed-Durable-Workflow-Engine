...
`scripts/dur050-run-generator-block.ps1`. It reads parsed Terraform outputs
and the `preparation-dry-run.json` produced by pilot preparation, validates
the generator ID, private database address, observer parameter, database
name, rendered config path and run arguments, then constructs the three
required environment exports and a single explicit `bash SCRIPT ...` command.
`-StagePlanOnly` prints the exact wrapped command without contacting AWS; a
real dispatch records its SSM command ID in the dispatch JSON.

Dispatch template (replace the placeholders with this cycle's recorded
...
pwsh -File scripts/dur050-run-generator-block.ps1 `
  ...
  -Mode unloaded -DatabaseName durable -BlockID unloaded-seq-8-01 -Family seq-8 `
  -OutputDirectory /var/tmp/dur050-unloaded-seq-8-01 `
  -RecordPath <cycle>/unloaded-seq-8-01.dispatch.json
```

| Runner | Dispatch mode and arguments |
|---|---|
| `scripts/dur050-warmup.sh` | Reset helper's `submit-eight-warmups` stage; generated by `dur050-reset-block.ps1` |
| `scripts/dur050-run-unloaded-block.sh` | `-Mode unloaded -BlockID ID -Family seq-8\|fanout-8` |
| `scripts/dur050-run-window.sh` | `-Mode window -RunID ID -Rate N -RunMode count\|duration -Value N` |
| `scripts/dur050-run-sink-check.sh` | `-Mode sink-check` |
| `scripts/dur050-capture-transaction-timings.sh` | App-host capture stages generated by the campaign tooling |
...
archive hash, chunk count and every metadata/chunk SSM command ID. Truncated,
missing or altered chunks fail before any destination directory is created.
Do not hand-write `-RemoteLines` for these standalone runners or attempt to
retrieve a block from `StandardOutputContent` as one response.
The remote tar archive remains at its recorded `/var/tmp/dur050-transfer-*.tar.gz`
path until host teardown, so an interrupted local transfer can be retried
