...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/m5-campaign.ps1
```

The output records the exact package, test pattern, exit code, start time, and
captured test output for every family. F12 remains the agent-specific M6
campaign. The M5 outage/restart evidence is produced separately by
