...
These are provisional weekly checkpoints, not promised dates. Reforecast after the first working durable activity.
...
- **User:** own design choices, approve paid scope, perform the human-approval demo.
...
| `docs/R092_STUDY_GUIDE.md` | Codex-performed engineering walkthroughs and claim-to-evidence links; not personal-fluency evidence |
...
- **Status:** DONE; round-40 Claude review has no blocking findings for reachable reviewed target `71fd54a`.
...
- **Known limits:** single-node Docker Desktop/WSL2 PostgreSQL evidence; the crash is a bounded local self-exiting process failure and the pause is a bounded renewal suspension; the harness fixes no scheduler/worker workload or offered-rate claim because it isolates lease mechanics; no multi-host, storage-failure, maximum-throughput, or production scheduler claim.
...
- **Goal:** measure how three frozen persistence intervals trade checkpoint writes and bytes against repeated pure computation after a bounded crash, while checking that every recovered run reaches the same deterministic final hash.
- **Scope:** one pure 200-chunk activity with activity-boundary-only persistence, a checkpoint every five chunks, or a checkpoint every chunk; each setting has one no-failure run and one crash-at-chunk-100 run for three repeats. The crash is an in-process activity-boundary failure followed by a fresh Engine recovery; it does not claim process-kill or external-effect semantics.
- **Protected boundaries:** preserve the section-14 three-settings/two-conditions/three-repeat matrix, one 200-chunk pure workload, correctness guarantees, release criteria, paid budgets, and the statement that checkpoint evidence does not establish atomicity for unrelated external effects. Do not start DUR-029 or DUR-033A from this task.
- **Acceptance scenarios:** every measured row records setting, failure condition, seed, work units per chunk, committed progress at failure, computed and recomputed chunks, checkpoint count/bytes, persistence telemetry, completion and recovery timings, final hash, terminal state, and cleanup. The artifact also aggregates all six cells across repeats, derives interval-based conclusions, and records limitations. No-failure rows must compute 200 chunks exactly; crash rows must show a real durable checkpoint prefix, recover after the attempt lease expires, and reconcile to the same hash. Incomplete or unreconciled rows fail the campaign.
- **Pilot/validation:** the pilot froze the chunk work, crash barrier, attempt lease, payload schema, and output hash before the 18 measured rows. Focused Go tests, the pilot and final PowerShell runner against PostgreSQL, repository race/static checks, formatting, PowerShell parsing, diff checks, and post-run namespace cleanup pass; the default Python check required a task-local pytest base directory because the host's shared temp directory is unreadable.
...
| DUR-031 — Interview evidence and walkthroughs | Map each proposed resume claim to code and a reproducible artifact. Codex explains and changes one lease/attempt rule, walks through the ambiguous-effect case, and independently reproduces one result. |
...
- **Status:** DONE for the engineering-evidence scope; Claude's round-47
  review returned `NO_BLOCKING_FINDINGS` for target `fab88ac`. D019 reassigns
  the three walkthroughs to Codex performance; R092 is WITHDRAWN by scope
  change, not satisfied.
...
- **Goal:** map proposed resume claims to reviewed implementation commits,
  ...
  commands; prepare explanations for lease fencing, ambiguous
  effects, and one independent result reproduction.
- **Scope:** documentation only in `docs/INTERVIEW_EVIDENCE.md`; no code,
  migration, experiment rerun, provider call, paid budget, external action,
  or protected-scope change. The agent must make the lease mutation only in a
...
- **D019 outcome:** the three exercises have dated, attributable Codex records
  in `docs/R092_STUDY_GUIDE.md`: the unsafe lease mutation failed as expected,
  the non-cooperating reconciliation test passed, and the checkpoint ratio
  reproduced as `7.5552`. This is engineering evidence, not evidence that the
  user personally performed or understands the exercises. R092 is WITHDRAWN by
  scope change, and PLAN's no-fabricated-authorship rule remains binding.
- **Known limitations:** R057, R083, and R088 were VERIFIED in round 50;
  R092 is WITHDRAWN by scope change under D019. The pack does not turn
  bounded local studies into production or multi-host claims.
...
- **Status:** DONE; Claude's committed round-49 review returned
  ...
  R092 is WITHDRAWN by scope change under D019; no personal-fluency claim is
  made.
...
- **Known limitations:** local Docker Desktop/WSL2 evidence remains bounded;
  ...
  R057, R083, and R088 were VERIFIED in round 50; R092 is WITHDRAWN by scope
  change under D019. The full review also records that the
...
- [x] Two scheduler replicas and Python worker pools.
- [x] Reviewed lease, attempt, dispatch, checkpoint, effect, and approval contracts.
- [x] Named failure traces and an independently checked effect ledger.
- [x] Required invariant/race/outage cases executed and reconciled.
...
- [x] Frozen throughput, safeguard-cost, dispatch-path, lease, and checkpoint studies for full v1.
- [x] Deterministic incident demo and held-out live-model evaluation for full v1.
- [x] All unknown effects, pending tests, and limited conclusions visible.
- [x] Build log, decision records, and claim-to-evidence table.
- [x] Codex walkthroughs completed.
...
R019 claim-retry coverage was mutation-verified by Claude in round 51 and is
no longer an unverified gap. M2 is DONE at reviewed code target `0d663c3`
...
DUR-023B, DUR-024, DUR-025, and DUR-021A are DONE. R057 was verified closed in
Claude's round-50 review as a nonblocking P3 evidence-labelling correction.
Keep the M0 contracts and
...
DUR-031 and M8 are DONE for their engineering-evidence scope. D019 reassigns
the three walkthrough exercises to Codex performance; R092 is WITHDRAWN by
scope change rather than satisfied, and no document may imply personal user
fluency.
The release pack separates the 48/48 validation campaign from its three
comparative headline findings, uses DUR-028 as the third comparative result,
and preserves R089's counts with rates.
...
round-49 verdict is `NO_BLOCKING_FINDINGS`. R092 is WITHDRAWN under D019, and
the user-authorized local annotated release tag is `v0.1.0` at `7e4137d`.
...
## 17. Portfolio extension selected after M8

The user first deferred R096 under D016, then authorized and superseded that
deferral for repair under D017. The repair preserves the in-transaction
lease-row lock and epoch fence. Implementation is committed at `41bb9bc`,
with the bounded-wait regression and scheduler-continuation evidence recorded
in the build log. Claude's round-55 review verified the repair; the historical
lock-held takeover campaign exclusion remains an artifact-scope label, not an
open repair. The current GitHub publication keeps this planning record local;
the public README and technical evidence remain the recruiter-facing surface.

### DUR-037 - Reproducible Linux CI

- **Status:** IN_PROGRESS
- **Dependencies:** existing accepted code; GitHub repository access; no R096,
  cloud, authentication, or paid-model dependency.
- **Goal:** run Go, Python, race, real PostgreSQL/Kafka smoke, deployed health,
  and offline M5 evidence checks on a fresh Linux hosted runner.
- **Scope:** add a GitHub Actions workflow using the existing PowerShell
  bootstrap/check/CI/archive commands; fail integration setup rather than skip;
  upload service diagnostics; keep live-model and long fault campaigns out of
  ordinary pull-request validation.
- **Acceptance:** a fresh `ubuntu-24.04` runner executes the static/unit job and
  the service-backed `ci.ps1 -WithServices -WithRace` job; failures retain
  Compose diagnostics; no secrets or paid calls are required.
- **Validation:** `git diff --check`; workflow YAML inspection; local
  `pwsh ./scripts/check.ps1` and `pwsh ./scripts/ci.ps1 -WithRace` where the
  current machine permits; hosted run URL and artifact retained after push.
- **Evidence:** `.github/workflows/ci.yml`, GitHub Actions run, and README
  reproduction section.
- **Known limits:** until a hosted run exists, the workflow is implementation
  evidence rather than a passing remote-CI claim. R096 is outside this task.

### DUR-041a - Local isolation and recovery campaign

- **Status:** READY_FOR_REVIEW
- **Dependencies:** existing local runtime/workers and PostgreSQL/Kafka; D017
  authorizes the R096 repair; no cloud or authentication dependency.
- **Goal:** measure local stale-epoch fencing, abrupt application-process
  recovery, and scheduler isolation/reconnect recovery with confirmed faults
  and useful progress.
- **Scope:** preserve the original `local-410041` owner-crash/owner-pause
  evidence; add separately labelled preserved-process arms for Docker pause /
  reconnect and for removing the original scheduler from the PostgreSQL
  network while it keeps running, allowing peer takeover, reconnecting the
  original process, and asserting its retained epoch cannot commit. Keep
  `lock_held_takeover` separately labelled even after the repair is accepted.
- **Acceptance:** zero superseded-owner mutations in each labelled arm,
  confirmed fault observations from Docker network/process state, useful
  affected-work progress where applicable, reconciled terminal rows, no
  leftover fixtures, and separate artifacts without overwriting `local-410041`.
- **Validation:** bounded lock-wait regression and scheduler-continuation
  tests; pause/reconnect and packet-level network-disconnect campaigns;
  existing serial service suite; offline checker; negative control with the
  bound removed; `git diff --check`.
- **Implementation:** `scripts/portfolio-local-recovery.ps1` preserves the
  original campaign; `scripts/dur041a-isolation.ps1` records either the pause
  arm or the packet-level PostgreSQL-network arm, including peer takeover,
  original container/PID continuity, reconnect, and stale-epoch rejection.
- **Evidence:** `experiments/portfolio/local-recovery/local-410041/` remains
  unchanged; the pause arm is published under
  `experiments/portfolio/local-recovery/isolation-410099/`, and the
  packet-level arm under
  `experiments/portfolio/local-recovery/network-410104/`. Repair and campaign
  commits are `41bb9bc`, `b45e34f`, `809db45`, `705f3c1`, and `1355372`, with
  the packet-level arm authorized by D018. R096 review status is left to
  Claude's status record.
- **Known limits:** local containers/processes are not independent hosts;
  results do not establish multi-host durability, database HA, or production
  availability. Hosted CI has now passed on GitHub Actions at the published
  `main` commit; no cloud or multi-host campaign was run.

### DUR-045 - Recruiter-facing evidence package

- **Status:** TODO
- **Dependencies:** accepted existing evidence, DUR-037 hosted run, and any
  reviewed DUR-041a artifact; no cloud or paid-model dependency.
- **Goal:** make the project quickly understandable and technically citable.
- **Scope:** README, architecture/failure diagram, one local recovery case
  study, short demo/reproduction instructions, claim-to-evidence links, and
  resume bullets with measured denominators and limitations.
- **Acceptance:** an unfamiliar reader can run the demo, locate the source
  evidence, explain the failure timeline, and distinguish local evidence from
  optional cloud work.
- **Validation:** fresh-source link check, one peer reproduction, Markdown
  link/whitespace checks, and review of each resume claim against an artifact.
- **Evidence:** README, technical report links, case study, and a reproducible
  demo transcript committed with the reviewed target.
- **Known limits:** this package is not a product launch, release authorization,
  or production-readiness claim. Its walkthroughs are Codex-performed
  engineering evidence and do not establish personal user fluency.

### Deferred scope

D020 selects an evidence-led completion program under the user's standing
authorization. The sequence is DUR-046 quality gates, DUR-042 deployed profiling,
DUR-040/041b private AWS recovery, DUR-043 operational recovery, then DUR-038
scoped authorization and the final DUR-045 presentation. Detailed sequencing,
estimated size, proof requirements and explicit exclusions are recorded in
`docs/COMPLETION_PLAN.md`. Existing measurement artifacts are preserved.

DUR-040, DUR-041b, DUR-038, DUR-039, DUR-042, DUR-043, and DUR-044 remain
unstarted or optional. Cloud provisioning requires a separate budget decision;
authentication is required before untrusted write exposure. R096's repair is
verified under D017; the historical local campaign still excludes its
lock-held takeover arm and does not imply multi-host liveness.

D021 makes Kubernetes required for the portfolio completion program and
supersedes the conditional Kubernetes language in PLAN:414. D022 authorizes a
$200 USD maximum for steps 4–8, including recovery drills and retained cloud
resources; it does not authorize Project 1's EKS clusters or paid model calls.
The nine-step sequence below is the active completion plan. Planning and study
notes remain in ignored docs; code, tests, workflows, scripts, and measured
experiment evidence remain public where their task records require them.

### DUR-047 - Completion checklist and incremental study corpus

- **Status:** TODO.
- **Dependencies:** accepted M8 baseline; no cloud, Kubernetes, authentication,
  or paid-model dependency.
- **Goal:** inventory implemented behavior against measured guarantees and make
  the project explainable without inflating claims.
- **Scope:** inspect the real OpenTelemetry depth in the engine path; map each
  selected extension to a bounded question and evidence requirement; create
  ignored `docs/STUDY_CORPUS/` chapters for leases/fencing, outbox/inbox,
  attempts/checkpoints, effects/approvals, fault checking, measurements, the
  incident agent, lock-versus-lease liveness, and limits/open questions. Each
  chapter records the mechanism, implementation map, alternatives, failure
  mechanism, exact reproduction, observed output, limitations, interview Q&A,
  and two-minute/deep explanations. Update it as later tasks land.
- **Acceptance:** every selected task has a bounded scope, named evidence path,
  reproducible command, and explicit claim boundary; no study or README claim
  relies on an unmeasured guarantee.
- **Validation:** `rg --files`; `rg -n "trace|Tracer|OTEL|otel" cmd internal python deploy`;
  `pwsh -File scripts/check.ps1`; `go test ./...`; `git diff --check`.
- **Evidence:** ignored `docs/STUDY_CORPUS/`; public README/evidence links added
  only when the corresponding result exists; local build-log entry.
- **Known limits:** the corpus is a teaching and planning record, not evidence
  by itself; external reproduction remains incomplete until another person runs
  it.

### DUR-046 - Mutation gate, fuzzing, property tests, and reference model

- **Status:** TODO.
- **Base:** `68dc607`.
- **Dependencies:** DUR-047; accepted safety regression tests; local Go/Python
  tools; PostgreSQL in an isolated disposable database; hosted Linux CI access.
- **Goal:** make critical safety regressions mechanically detectable and add
  independent state-machine and parser robustness checks.
- **Scope:** create one isolated mutation record per named guard, including the
  lease fence, approval resource guards, wakeup lease lock, pool/Compose
  `lock_timeout`, `lockLease` error classification, M5 boundary/outbox rules,
  and the canary scanner. Build the PowerShell and Linux gates in a scratch
  export, never the working tree. Add an independent contract-derived
  reference model, stateful property sequences, shrinking, and fuzz targets for
  request JSON, workflow graphs, and fault traces. Extend CI with bounded fuzz,
  `govulncheck`, artifacts, the mutation gate, and integration-skip detection
  only where integration execution is required. Add a CI badge only after a
  hosted run exists.
- **Acceptance:** baseline passes; each named mutation fails its intended test
  with the expected semantic message; restored baseline passes; build failures,
  skips, timeouts, or unrelated failures never count as detections; the model
  does not reuse production transition validation as its oracle; fuzz/property
  checks exercise meaningful orderings. Finding a new defect is valuable but is
  not required for task completion.
- **Validation:** `pwsh -File scripts/mutation-gate.ps1`; `bash scripts/mutation-gate.sh`;
  `go test -race ./internal/...`; bounded named fuzz commands such as
  `go test -run=^$ -fuzz=Fuzz -fuzztime=30s ./internal/...`; `go vet ./...`;
  `uv run ruff check .`; `uv run mypy python`; `uv run pytest`; hosted CI at
  the exact committed target.
- **Evidence:** `mutations/`; mutation results JSON; reference-model and fuzz
  tests; `.github/workflows/ci.yml`; CI artifact and run URL; ignored corpus
  chapter on independent checking.
- **Known limits:** mutation coverage is a selected safety sample, not proof of
  all regressions; local runs do not establish hosted CI until executed there.

### DUR-048 - Deployed measurement harness and telemetry

- **Status:** TODO.
- **Dependencies:** DUR-046; local PostgreSQL/Kafka Compose stack; existing
  runtime, API, relay, and Python worker paths.
- **Goal:** measure the real HTTP-to-worker execution path and make one normal
  and one recovery trace reconstructable.
- **Scope:** inspect and extend the existing OTLP collector with one trace
  backend; add guarded pprof; build an open-loop HTTP load generator with fixed
  worker capacity, warmup, repeats, per-request latency, and accepted-versus-
  terminal reconciliation. Propagate W3C context across API, scheduler,
  outbox, Kafka, worker, receipt, and approved effect execution, using links for
  asynchronous work. Do not emit an effect span unless the approved effect
  service actually runs.
- **Acceptance:** a normal execution and a retry/recovery execution can be
  reconstructed end to end; incomplete work fails the harness; fixed worker
  capacity is recorded; local numbers are labelled pilot evidence.
- **Validation:** `docker compose --env-file .env -f deploy/local/compose.yaml config`;
  `pwsh -File scripts/bootstrap.ps1 -StartServices`;
  `pwsh -File scripts/dur042-load.ps1 -Mode pilot`;
  `go test -race ./...`; `go vet ./...`; `git diff --check`.
- **Evidence:** `experiments/m8/dur042-pilot/` with protocol, raw rows,
  summaries, profiles, trace export, reconciliation, and source commit;
  local corpus chapter and concise README pointer.
- **Known limits:** local pilot data is not an AWS or end-to-end production
  throughput claim; asynchronous trace timing must not be inferred from host
  wall clocks without a declared clock model.

### DUR-049 - AWS multi-host recovery

- **Status:** TODO.
- **Dependencies:** DUR-048 pilot; D022 cloud cap; existing `portfolio-dev`
  AWS identity; no use of Project 1's clusters.
- **Goal:** measure lease fencing and useful recovery across independent hosts,
  including a real network partition and host-level failure.
- **Scope:** provision two application hosts and one dependency host with
  Terraform, private access, pinned image digests, bounded resources, and
  teardown. Run preserved-process network isolation/reconnect, isolation while
  holding the lease-row lock, and abrupt application-host loss with work in
  flight. Recalibrate remote lease settings and keep fault-command time separate
  from takeover and useful-progress time.
- **Acceptance:** provisioning/destruction reproduce; each fault is confirmed
  from network/process/backend state; original-process stale writes are fenced;
  useful affected-work progress and outstanding obligations are measured;
  resources are cleaned and actual cost is reported. Do not infer database-host
  durability from an application-host failure.
- **Validation:** `terraform -chdir=deploy/aws fmt -check -recursive`;
  `terraform -chdir=deploy/aws init -backend=false`;
  `terraform -chdir=deploy/aws validate`; `terraform -chdir=deploy/aws plan`;
  `pwsh -File scripts/dur043-multihost.ps1 -Scenario all`;
  `go test -race ./...`; offline invariant checker; teardown verification.
- **Evidence:** `experiments/portfolio/cloud-recovery/<id>/` containing topology,
  cost, digests, fault confirmations, owner/epoch rows, stale-write counts,
  takeover/useful-progress timings, reconciliation, and cleanup report.
- **Known limits:** this is a bounded topology and workload, not a production
  HA or multi-region claim; database-host durability is a separate drill.

### DUR-050 - Cloud capacity study and one profile-driven optimization

- **Status:** TODO.
- **Dependencies:** DUR-048; DUR-049 topology; fixed worker capacity; frozen
  workload and protocol; D022.
- **Goal:** identify and measure one bottleneck on the AWS deployment without
  confounding scheduler count, worker capacity, or environment.
- **Scope:** freeze the workload and SLOs before final runs; collect baseline
  CPU, memory, latency, database pressure, backlog, and resource data; profile;
  change exactly one profiled bottleneck such as pool size, RuntimeWork query/
  index, relay batch size, or scheduler tick; rerun on identical resources.
- **Acceptance:** matched before/after results have correctness and reconciliation
  checks unchanged, repeat dispersion reported, and unresolved comparisons left
  unresolved. A performance improvement is not required. Worker capacity stays
  fixed.
- **Validation:** `pwsh -File scripts/dur042-load.ps1 -Mode cloud-baseline`;
  `pwsh -File scripts/dur042-load.ps1 -Mode cloud-after`;
  `go test -race ./...`; `go vet ./...`; `git diff --check`.
- **Evidence:** `experiments/m8/dur042-cloud-baseline/` and
  `experiments/m8/dur042-cloud-after/` with frozen protocol, profiles, raw
  measurements, comparison, cost, and correctness reconciliation.
- **Known limits:** conclusions apply only to the recorded AWS topology,
  workload, and worker capacity; no maximum-throughput claim is implied.

### DUR-051 - Overload and bounded soak

- **Status:** TODO.
- **Dependencies:** DUR-048; DUR-050 frozen measurement harness; D022; fixed
  worker capacity.
- **Goal:** characterize admission, backpressure, queue growth, and stability
  under open-loop overload and a sustained run.
- **Scope:** ramp offered rate past capacity with explicit accepted/rejected
  accounting; add bounded admission/queue controls only where the experiment
  demonstrates their absence; run an initial approximately one-hour soak and
  extend only for a named stability question such as resource leakage or
  delayed reconciliation.
- **Acceptance:** every accepted workflow is accounted for; memory, goroutine,
  connection, and storage behavior is bounded or explained; backlog growth and
  drain-to-zero are measured; worker capacity remains fixed.
- **Validation:** `pwsh -File scripts/dur044-soak.ps1 -Mode overload`;
  `pwsh -File scripts/dur044-soak.ps1 -Mode soak -Duration 1h`;
  `go test -race ./...`; `go vet ./...`; cleanup and cost verification.
- **Evidence:** `experiments/portfolio/operations/dur044-overload/` and
  `experiments/portfolio/operations/dur044-soak/` with protocol, resource
  trends, backlog-age data, reconciliation, teardown, and actual cost.
- **Known limits:** a one-hour run is a bounded stability observation, not a
  universal capacity or long-term production reliability guarantee.

### DUR-052 - Operations, recovery drills, and access controls

- **Status:** TODO.
- **Dependencies:** DUR-048 telemetry; DUR-049 deployment; D022; no untrusted
  write exposure before authentication is accepted.
- **Goal:** make failures diagnosable from telemetry and protect authority-
  changing API actions.
- **Scope:** provision dashboards and alerts for oldest outstanding work, failed
  relay passes, and stalled progress; trigger and diagnose each from telemetry
  alone. Implement separate drills for PostgreSQL restart with retained storage,
  isolated backup restore, Kafka restart/rebalance, and rolling application
  upgrade. Treat permanent disk loss as a disposable-storage experiment with
  declared backup point, measured RPO/RTO, and recovered/missing work; do not
  promise zero loss without a supporting replication/backup guarantee. Split
  liveness/readiness and implement full drain semantics before the drills. Add
  scoped tokens/roles, durable authenticated identity, and attack tests before
  untrusted writes.
- **Acceptance:** each drill has a distinct failure model and reconciliation;
  dashboards/alerts fire and are diagnosed; graceful drain and forced
  termination both leave correct recoverable state; auth rejects forged
  identity, wrong role, expired/rotated tokens, worker-token reuse, and approval
  bypass.
- **Validation:** `pwsh -File scripts/drill-postgres-restart.ps1`;
  `pwsh -File scripts/drill-backup-restore.ps1`;
  `pwsh -File scripts/drill-kafka-rebalance.ps1`;
  `pwsh -File scripts/drill-rolling-upgrade.ps1`;
  `go test -race ./...`; `go vet ./...`; `uv run pytest`; alert and trace
  export inspection; teardown verification.
- **Evidence:** `deploy/observability/`; `docs/PERMISSION_MATRIX.md` (local
  process notes remain ignored); `experiments/portfolio/operations/<id>/` per
  drill; concise public incident summary beside evidence; durable trace export.
- **Known limits:** these drills do not establish multi-region failover,
  database HA, or production security beyond the declared private deployment.

### DUR-053 - Required Kubernetes deployment and recovery campaign

- **Status:** TODO.
- **Dependencies:** DUR-048 deployed runtime; DUR-052 split health/drain
  contract; D021 Kubernetes scope; D022 cloud cap. Do not use Project 1 EKS.
- **Goal:** demonstrate reproducible Kubernetes operations, graceful lifecycle,
  worker scaling, and recovery of durable work.
- **Scope:** provision a dedicated cluster through Terraform, choosing EKS or
  self-managed explicitly; record node topology, Kubernetes/add-on versions,
  dependency connectivity, cost, and teardown. Build Kustomize base/local/cloud
  overlays for runtime and workers with ConfigMap, Secret reference, Services,
  probes, resources, termination grace, and preStop. Install KEDA reproducibly
  and add Helm after manifests stabilize. Keep PostgreSQL/Kafka outside the
  cluster initially. Implement SIGTERM readiness-first drain, stop new claims,
  finish/report in-flight work within a bound, then release ownership; retain
  recoverable state on forced termination. Validate the scaling signal against
  durable runnable work and active claims because Kafka lag alone is not
  workflow backlog; record partition/slot/replica/stabilization limits.
- **Acceptance:** a clean environment reproduces from source; confirmed process
  death, live-process isolation/reconnect, graceful drain/rolling update with
  rollback, and scaling each produce measured fault/progress evidence plus the
  existing checker verdict. A dead process is not used as stale-write evidence;
  retained-identity fencing is a separate case. Autoscaling is disabled during
  DUR-048–DUR-051 and enabled only in its labelled scaling experiment.
- **Validation:** `terraform -chdir=deploy/aws fmt -check -recursive`;
  `terraform -chdir=deploy/aws validate`; `kubectl kustomize deploy/k8s/overlays/local`;
  `kubectl apply --dry-run=client -k deploy/k8s/overlays/local`;
  `helm lint deploy/helm/durable-engine`; `kubectl apply -k deploy/k8s/overlays/cloud`;
  `pwsh -File scripts/k8s-pod-loss-drill.ps1 -Scenario all`;
  `kubectl get pods,deployments,events`; offline invariant checker; teardown.
- **Evidence:** `deploy/k8s/`, `deploy/helm/durable-engine/`, cluster/topology
  and cost manifests, and `experiments/portfolio/k8s-recovery/<id>/` with fault
  confirmation, process/container observations, epochs, progress, backlog/lag
  curves, checker results, rollback, and cleanup.
- **Known limits:** evidence is bounded to the chosen cluster, dependencies,
  workload, and fault model; it is not a claim of multi-region Kubernetes,
  managed-database durability, or production readiness.

### DUR-054 - Reproducible release and independent reproduction

- **Status:** TODO.
- **Dependencies:** DUR-046 through DUR-053 reviewed where applicable; a
  releasable image and evidence set; no new cloud or model budget.
- **Goal:** make the verified system and its evidence reproducible by another
  person and easy to evaluate.
- **Scope:** immutable multi-architecture images by digest, SBOM, vulnerability
  report, signature and deployment-path verification; Makefile and Linux shell
  entry points; changelog; short recovery demonstration; prompt/retrieval/
  citation regression tests; README focused on the strongest bounded findings.
  Keep the full study corpus and internal write-ups ignored.
- **Acceptance:** an independent person—not an agent—follows the quickstart on
  a clean machine, deploys released images, verifies a claim, and exercises a
  rollback; friction is recorded and fixed. No public claim exceeds its source
  population, configuration, or failure model.
- **Validation:** `make test`; `make demo`; `docker buildx bake`;
  `syft <image>@<digest>`; `grype <image>@<digest>`; `cosign verify <image>@<digest>`;
  Linux quickstart from a clean checkout; prompt/retrieval/citation regression
  tests; README link and whitespace checks.
- **Evidence:** release workflow and artifacts, `CHANGELOG.md`, immutable image
  digests, SBOM/signature/scan records, demo recording, public README/evidence
  links, and an independent reproduction report.
- **Known limits:** release evidence does not authorize a production launch or
  imply universal exactly-once execution; hosted CI and external reproduction
