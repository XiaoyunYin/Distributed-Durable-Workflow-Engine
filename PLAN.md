...
- **Implementation target:** `d08fb13` (dispatch runner, isolation/recovery fixes, relay timing evidence, and final artifact provenance).
...
- **Evidence:** `experiments/m7/dur035/results.json` is `PASS` from `d08fb13`, with 12 runs (four configurations x three repeats), 144 measured workflows plus 48 warmups, zero pending workflows, finite stage timings, 64 Kafka relay publications and 16 Kafka task receives/commits per Kafka repeat, and zero broker traffic in the two direct arms. The report withholds cost effects (`cost_effects_resolved: false`) because this is a three-repeat bounded study and the observed spread is descriptive rather than a promoted performance claim.
