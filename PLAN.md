...
- **Status:** IN_PROGRESS; implementation and the four-profile evidence run are being prepared from the accepted DUR-026 closeout.
...
- **Planned evidence:** `experiments/m7/dur034/results.json`, generated by `scripts/m7-dur034.ps1` from a clean committed worktree, plus the implementation record and build log. The command must sweep only the reserved DUR-034 namespaces before and after the study.
...
DUR-034 is now IN_PROGRESS from base `611ff26`. Its four-profile,
three-repeat safeguard-cost protocol is fixed by section 14B; implementation
and evidence must be committed before requesting Claude review. The immediate
next action is to run `scripts/m7-dur034.ps1` from the clean target worktree,
then hand off the committed implementation and
