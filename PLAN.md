...
- **Status:** IN_PROGRESS; implementation is being prepared for review.
...
- **Acceptance validation:** run `powershell.exe -NoProfile -ExecutionPolicy Bypass -File ./scripts/m7-readiness.ps1 -StartServices`; verify the artifact is `PASS`, the host is Linux, all real dependencies are healthy, the normal and fault checks pass, and the worktree remains clean except for the generated evidence before the handoff commit.
